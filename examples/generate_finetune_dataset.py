...
geo_letters = [chr(e) for e in range(4304, 4304 + 33)]
geo_letters.append(' ')
...
def insert_characters(text, augmentation_probability=0.01):
    ...
    for char in text:
        if random.random() <= augmentation_probability and char not in digits:            
            modifyed_line.append(random.choice(geo_letters))
        modifyed_line.append(char)
    return "".join(modifyed_line)
...
if __name__ == "__main__":
    data_file = "/home/penguin/GeorgianWritingWizard/language_engine/examples/data.txt" #"data/en.wikidump.processed.24m.txt" #
    language = "ge"
    ...
    tokenizer = AutoTokenizer.from_pretrained("ZurabDz/BartGeo")
    with open(language+".csv","w",encoding='utf-8') as output:        
        with open(data_file,'r') as file:
            for line in tqdm(file, total=num_lines):
                ...
                if tokenizer_check_if_text_too_long(line,tokenizer,max_length=64):
                ...
                new_line = ' '.join(new_line.strip().split())
                line = ' '.join(line.strip().split())
                output.write(f'"{new_line.lower()}","{line.lower()}"\n') 

    os.system(f"echo \"text,summary\" > {language}.train.csv")
    num_lines = sum(1 for line in open(f"{language}.csv",'r'))
    os.system(f"head -n {num_lines-8000} {language}.csv >> {language}.train.csv")
    os.system(f"echo \"text,summary\" > {language}.test.csv")
    os.system(f"tail -n 8000 {language}.csv >> {language}.test.csv")